...
class Classes:
    ...
    def create(self, name, teacher):
        try:
            ...
            insert_query = """ INSERT INTO classes (name, teacher) VALUES (%s, %s)"""
            item_tuple = (name, teacher)
...
class Teachers:
    ...
    def create(self, name, surname):
        try:
            connection = psycopg2.connect(user="postgres",
                                          ...
                                          database="school")
            cursor = connection.cursor()
            insert_query = """ INSERT INTO teachers (name, surname) VALUES (%s, %s)"""
            item_tuple = (name, surname)
            cursor.execute(insert_query, item_tuple)
            connection.commit()
            print("Entity inserted")
    ...
    def delete(self, id):
        ...
        try:
            ...
            cursor = connection.cursor()
            cursor.execute(
                "Delete from classes WHERE teacher = %s; Delete from groups WHERE teacher = %s; Delete from teachers WHERE id = %s;", [id, id, id])
            connection.commit()
            count = cursor.rowcount
            print(count, "Entity deleted")
        except (Exception, Error) as error:
...
class Groups:
    ...
    def create(self, name, year, num_of_exc, teacher):
        try:
            connection = psycopg2.connect(user="postgres",
                                          password="yuliya200216",
                                          host="127.0.0.1",
                                          ...
                                          database="school")
            cursor = connection.cursor()
            insert_query = """ INSERT INTO groups (name, year, num_of_exc, teacher) VALUES (%s, %s, %s, %s)"""
            item_tuple = (name, year, num_of_exc, teacher)
            cursor.execute(insert_query, item_tuple)
            connection.commit()
            print("Entity inserted")
    ...
    def read(self, id):
        ...
        try:
            ...
            print("Result:")
            for row in records:
                print("Id = ", row[0], "Name = ", row[1], "Year = ", row[2],
                      "Number of excelent students = ", row[3], "Teacher  = ", row[4])
        except (Exception, Error) as error:
            print("Error with PostgreSQL", error)
        finally:
            if connection:
